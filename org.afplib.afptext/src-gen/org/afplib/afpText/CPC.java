/**
 * generated by Xtext 2.19.0
 */
package org.afplib.afpText;


/**
 * <!-- begin-user-doc -->
 * A representation of the model object '<em><b>CPC</b></em>'.
 * <!-- end-user-doc -->
 *
 * <p>
 * The following features are supported:
 * </p>
 * <ul>
 *   <li>{@link org.afplib.afpText.CPC#getDefCharID <em>Def Char ID</em>}</li>
 *   <li>{@link org.afplib.afpText.CPC#getPrtFlags <em>Prt Flags</em>}</li>
 *   <li>{@link org.afplib.afpText.CPC#getCPIRGLen <em>CPIRG Len</em>}</li>
 *   <li>{@link org.afplib.afpText.CPC#getVSCharSN <em>VS Char SN</em>}</li>
 *   <li>{@link org.afplib.afpText.CPC#getVSChar <em>VS Char</em>}</li>
 *   <li>{@link org.afplib.afpText.CPC#getVSFlags <em>VS Flags</em>}</li>
 * </ul>
 *
 * @see org.afplib.afpText.AfpTextPackage#getCPC()
 * @model
 * @generated
 */
public interface CPC extends structuredField
{
  /**
   * Returns the value of the '<em><b>Def Char ID</b></em>' attribute.
   * <!-- begin-user-doc -->
   * <p>
   * If the meaning of the '<em>Def Char ID</em>' attribute isn't clear,
   * there really should be more of a description here...
   * </p>
   * <!-- end-user-doc -->
   * @return the value of the '<em>Def Char ID</em>' attribute.
   * @see #setDefCharID(String)
   * @see org.afplib.afpText.AfpTextPackage#getCPC_DefCharID()
   * @model
   * @generated
   */
  String getDefCharID();

  /**
   * Sets the value of the '{@link org.afplib.afpText.CPC#getDefCharID <em>Def Char ID</em>}' attribute.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param value the new value of the '<em>Def Char ID</em>' attribute.
   * @see #getDefCharID()
   * @generated
   */
  void setDefCharID(String value);

  /**
   * Returns the value of the '<em><b>Prt Flags</b></em>' attribute.
   * <!-- begin-user-doc -->
   * <p>
   * If the meaning of the '<em>Prt Flags</em>' attribute isn't clear,
   * there really should be more of a description here...
   * </p>
   * <!-- end-user-doc -->
   * @return the value of the '<em>Prt Flags</em>' attribute.
   * @see #setPrtFlags(Integer)
   * @see org.afplib.afpText.AfpTextPackage#getCPC_PrtFlags()
   * @model
   * @generated
   */
  Integer getPrtFlags();

  /**
   * Sets the value of the '{@link org.afplib.afpText.CPC#getPrtFlags <em>Prt Flags</em>}' attribute.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param value the new value of the '<em>Prt Flags</em>' attribute.
   * @see #getPrtFlags()
   * @generated
   */
  void setPrtFlags(Integer value);

  /**
   * Returns the value of the '<em><b>CPIRG Len</b></em>' attribute.
   * <!-- begin-user-doc -->
   * <p>
   * If the meaning of the '<em>CPIRG Len</em>' attribute isn't clear,
   * there really should be more of a description here...
   * </p>
   * <!-- end-user-doc -->
   * @return the value of the '<em>CPIRG Len</em>' attribute.
   * @see #setCPIRGLen(Integer)
   * @see org.afplib.afpText.AfpTextPackage#getCPC_CPIRGLen()
   * @model
   * @generated
   */
  Integer getCPIRGLen();

  /**
   * Sets the value of the '{@link org.afplib.afpText.CPC#getCPIRGLen <em>CPIRG Len</em>}' attribute.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param value the new value of the '<em>CPIRG Len</em>' attribute.
   * @see #getCPIRGLen()
   * @generated
   */
  void setCPIRGLen(Integer value);

  /**
   * Returns the value of the '<em><b>VS Char SN</b></em>' attribute.
   * <!-- begin-user-doc -->
   * <p>
   * If the meaning of the '<em>VS Char SN</em>' attribute isn't clear,
   * there really should be more of a description here...
   * </p>
   * <!-- end-user-doc -->
   * @return the value of the '<em>VS Char SN</em>' attribute.
   * @see #setVSCharSN(Integer)
   * @see org.afplib.afpText.AfpTextPackage#getCPC_VSCharSN()
   * @model
   * @generated
   */
  Integer getVSCharSN();

  /**
   * Sets the value of the '{@link org.afplib.afpText.CPC#getVSCharSN <em>VS Char SN</em>}' attribute.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param value the new value of the '<em>VS Char SN</em>' attribute.
   * @see #getVSCharSN()
   * @generated
   */
  void setVSCharSN(Integer value);

  /**
   * Returns the value of the '<em><b>VS Char</b></em>' attribute.
   * <!-- begin-user-doc -->
   * <p>
   * If the meaning of the '<em>VS Char</em>' attribute isn't clear,
   * there really should be more of a description here...
   * </p>
   * <!-- end-user-doc -->
   * @return the value of the '<em>VS Char</em>' attribute.
   * @see #setVSChar(Integer)
   * @see org.afplib.afpText.AfpTextPackage#getCPC_VSChar()
   * @model
   * @generated
   */
  Integer getVSChar();

  /**
   * Sets the value of the '{@link org.afplib.afpText.CPC#getVSChar <em>VS Char</em>}' attribute.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param value the new value of the '<em>VS Char</em>' attribute.
   * @see #getVSChar()
   * @generated
   */
  void setVSChar(Integer value);

  /**
   * Returns the value of the '<em><b>VS Flags</b></em>' attribute.
   * <!-- begin-user-doc -->
   * <p>
   * If the meaning of the '<em>VS Flags</em>' attribute isn't clear,
   * there really should be more of a description here...
   * </p>
   * <!-- end-user-doc -->
   * @return the value of the '<em>VS Flags</em>' attribute.
   * @see #setVSFlags(Integer)
   * @see org.afplib.afpText.AfpTextPackage#getCPC_VSFlags()
   * @model
   * @generated
   */
  Integer getVSFlags();

  /**
   * Sets the value of the '{@link org.afplib.afpText.CPC#getVSFlags <em>VS Flags</em>}' attribute.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param value the new value of the '<em>VS Flags</em>' attribute.
   * @see #getVSFlags()
   * @generated
   */
  void setVSFlags(Integer value);

} // CPC
